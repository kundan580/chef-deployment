...
def GenerateConfig(context):
    ...
    keyFile = context.properties['keyFile']
    serverName = context.properties["serverName"]
    ...
    resources.append({
        ...
        'properties': {
            ...
            'tags': {'items': ["ssh-server", "http-server", "all-ports"]},
            'metadata': {
                'items': [{
                          'key': 'startup-script',
                          'value': """
                                ...
                                """.format(
                                    ...
                                    password = password,    
                                    sshpubkey = sshpubkey,
                                    context = '\n'.join([' '*32 + i for i in context.imports["workstation_startup_script.sh"].split('\n')])
                                    )
                          },
                          {
                          'key': 'username',
    ...
    outputs.append({
      'name': 'externalIp',
      'value': '$(ref.{}.networkInterfaces[0].accessConfigs[0].natIP)'.format(name)
      })
